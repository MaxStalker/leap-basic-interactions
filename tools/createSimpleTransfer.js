...
async function main() {
  ...
  try {
    spendingCondition = require('./../build/contracts/SimpleLock.json');
  } catch (e) {
  ...
  codeBuf = spendingCondition.deployedBytecode
    .replace(RECEIVER_PLACEHOLDER, RECEIVER.replace('0x', '').toLowerCase())

  codeHash = ethUtil.ripemd160(codeBuf);
  spAddr = '0x' + codeHash.toString('hex');
  msgData = abi.functions.resolve.encode([]);
